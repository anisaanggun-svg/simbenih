...
@section("title", $mode == 'lihat' ? 'Lihat Konsep Label Standart - Sertifikasi' : ($mode == 'kosong' ? 'Edit Konsep Label Standart - Sertifikasi' : 'Form Konsep Label Standart - Sertifikasi'))
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    @if($mode == 'lihat')
                        Lihat Konsep Label Standart
                    @elseif($mode == 'kosong')
                        Edit Konsep Label Standart
                    @else
                        Form Konsep Label Standart
                    @endif
                </h3>
                <div class="card-tools">
                    @if($mode == 'lihat' || $mode == 'kosong')
                    <a href="https://daftar.bpsbjatim.com/simbenihkonseplabelstandart/{{ $konsep->print_hash ?? 'placeholder' }}/541" class="btn btn-sm btn-info" target="_blank">
                        <i class="fas fa-print mr-1"></i> Print Laporan
                    </a>
                    @endif
                    <a href="{{ url('') }}/admin/sertifikasi/label_standart" class="btn btn-sm btn-outline-secondary ml-1">
                        <i class="fas fa-arrow-left mr-1"></i> Kembali ke Daftar Konsep Label Standart
                    </a>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div class="info-box">
                    <div>
                        @if($mode == 'lihat')
                            Menampilkan detail data Konsep Label Standart
                        @else
                            User Melakukan Pengisian Form untuk mendapatkan No. Seri Label Standart
                        @endif
                    </div>
                </div>
                ...
                <div class="fieldset-custom">
                    <form action="@if($mode == 'create' || $mode == 'kosong')
                        {{ url('') }}/admin/sertifikasi/label_standart/{{ $mode == 'create' ? 'insert' : 'update' }}/{{ $id }}{{ $mode == 'kosong' ? '/' . ($a ?? '') : '' }}
                    @else
                        #
                    @endif" method="POST" name="form_konsep_label_standart" id="formKonsepLabelStandart">
                        @csrf
                        @if($mode == 'kosong')
                            @method('POST')
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control readonly-field" name="no_lab" id="no_lab"
                                        value="{{ $rekomendasi->no_lab ?? $konsep->no_lab ?? '' }}" readonly>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="warna_label" id="warna_label" class="form-control" {{ $mode == 'lihat' ? 'disabled' : 'required' }}>
                                        <option value="">Pilih warna label</option>
                                        <option value="Ungu" {{ ($konsep->warna_label ?? '') == 'Ungu' ? 'selected' : '' }}>Ungu</option>
                                        <option value="Putih" {{ ($konsep->warna_label ?? '') == 'Putih' ? 'selected' : '' }}>Putih</option>
                                        <option value="Biru" {{ ($konsep->warna_label ?? '') == 'Biru' ? 'selected' : '' }}>Biru</option>
                                    </select>
                                </div>
                        ...
                        <!-- Data Mutu Benih -->
                        <div class="fieldset-custom">
                            <legend>| Data Mutu Benih |</legend>
                            <div class="row">
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="KADAR_AIR_KOLAB">Kadar Air</label>
                                        <input type="text" class="form-control" name="KADAR_AIR_KOLAB" id="KADAR_AIR_KOLAB"
                                            value="{{ $konsep->kadar_air ?? old('KADAR_AIR_KOLAB', '') }}"
                                            {{ $mode == 'lihat' ? 'readonly' : 'required' }}>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="BENIH_MURNI_KOLAB">Benih Murni</label>
                                        <input type="text" class="form-control" name="BENIH_MURNI_KOLAB" id="BENIH_MURNI_KOLAB"
                                            value="{{ $konsep->benih_murni ?? old('BENIH_MURNI_KOLAB', '') }}"
                                            {{ $mode == 'lihat' ? 'readonly' : 'required' }}>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="GULMA_KOLAB">Gulma</label>
                                        <input type="text" class="form-control" name="GULMA_KOLAB" id="GULMA_KOLAB"
                                            value="{{ $konsep->gulma ?? old('GULMA_KOLAB', '') }}"
                                            {{ $mode == 'lihat' ? 'readonly' : 'required' }}>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="KOTORAN_BENIH_KOLAB">Kotoran Benih</label>
                                        <input type="text" class="form-control" name="KOTORAN_BENIH_KOLAB" id="KOTORAN_BENIH_KOLAB"
                                            value="{{ $konsep->kotoran_benih ?? old('KOTORAN_BENIH_KOLAB', '') }}"
                                            {{ $mode == 'lihat' ? 'readonly' : 'required' }}>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="DAYA_TUMBUH_KOLAB">Daya Tumbuh</label>
                                        <input type="text" class="form-control" name="DAYA_TUMBUH_KOLAB" id="DAYA_TUMBUH_KOLAB"
                                            value="{{ $konsep->daya_tumbuh ?? old('DAYA_TUMBUH_KOLAB', '') }}"
                                            {{ $mode == 'lihat' ? 'readonly' : 'required' }}>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            @endif
                            <a href="{{ url('') }}/admin/sertifikasi/label_standart" class="btn btn-secondary btn-form">
