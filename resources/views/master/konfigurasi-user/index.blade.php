...
@section("title", "Master Konfigurasi User - Data Master")
...
<style>
    #konfigurasiUserTable th { white-space: nowrap; }
    #konfigurasiUserTable td { vertical-align: middle; padding: 8px 10px; }
    .card-tools {
        display: flex;
        gap: 0.5rem;
        flex-wrap: wrap;
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Data Master &raquo; Manajemen Konfigurasi User</h3>
                <div class="card-tools">
                    <a href="{{ route('master.konfigurasi-user.create') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="input-group input-group-sm ml-2" style="width: 220px;">
                        ...
                        <input type="text" id="searchBox" class="form-control" placeholder="kata kunci pencarian" aria-controls="konfigurasiUserTable">
                    </div>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div class="table-responsive">
                    <table id="konfigurasiUserTable" class="table table-bordered table-striped table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th width="220" class="text-center">Nama Konfigurasi</th>
                                <th>Nilai</th>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p id="modalKonfirmasiHapusMessage">Apakah Anda yakin ingin menghapus data Konfigurasi User ini?</p>
...
<script>
    $(function () {
        var table = $('#konfigurasiUserTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "{{ route('master.konfigurasi-user.grid') }}",
                "type": "GET",
            ...
            "columns": [
                ...
                { "data": "nama_konfigurasi", "className": "text-center" },
                {
                    "data": "nilai_konfigurasi",
                    "className": "nilai-cell",
                    "render": function (data, type, row) {
                        var val = (data === null || data === undefined || data === '') ? '<span class="text-muted">(kosong)</span>' : $('<div>').text(data).html();
                        return val;
                    }
                },
                {
                    ...
                    "width": "90",
                    "render": function (data) {
                        var editUrl = '{{ url("admin/master/konfigurasi-user/edit") }}/' + data;
                        var deleteUrl = '{{ url("admin/master/konfigurasi-user/delete") }}/' + data;
        ...
        $('#konfigurasiUserTable tbody').on('change', '.checkItem', function () {
            var id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $('#checkAll').on('change', function () {
            ...
            $('#konfigurasiUserTable tbody .checkItem').prop('checked', isChecked);
            syncCheckAllState();
        });

        // Terapkan ulang status pilihan setelah DataTables redraw
        table.on('draw', function () {
            $('#konfigurasiUserTable tbody .checkItem').each(function () {
                var id = $(this).val();
        ...
        $(document).on('click', '#konfigurasiUserTable .btn-hapus-satu', function (e) {
            e.preventDefault();
            var url = $(this).attr('href');
            $('#modalKonfirmasiHapusMessage').text(
                'Apakah Anda yakin ingin menghapus data Konfigurasi User ini? ' +
    ...
    function hapusData() {
        ...
        $('#modalKonfirmasiHapusMessage').text(
            ...
            ' data Konfigurasi User yang dipilih? Tindakan ini tidak dapat dibatalkan.'
        );
    ...
    $(document).on('click', '#btnKonfirmasiHapus', function () {
        ...
        if (mode === 'single') {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('master.konfigurasi-user.delete') }}",
