...
@section("title", "Master Penyakit - Data Master")
...
<style>
    #penyakitTable th { white-space: nowrap; }
    #penyakitTable td { vertical-align: middle; padding: 8px 10px; }
    .card-tools {
        display: flex;
        gap: 0.5rem;
        flex-wrap: wrap;
    ...
    #penyakitTable .badge.badge-success {
        background-color: #dff3e1 !important;
        color: #2e7d32 !important;
        border: 1px solid #c8e6c9;
    }
    #penyakitTable .badge.badge-danger {
        background-color: #fde2e2 !important;
        color: #c62828 !important;
        border: 1px solid #efcfcf;
    }
    /* Soft colors for Kelompok Penyakit badges */
    #penyakitTable .badge.badge-warning {
        background-color: #fff8e1 !important;
        color: #f57f17 !important;
        border: 1px solid #ffecb3;
    }
    #penyakitTable .badge.badge-info {
        background-color: #e1f5fe !important;
        color: #0277bd !important;
        border: 1px solid #b3e5fc;
    }
    #penyakitTable .badge.badge-primary {
        background-color: #e3f2fd !important;
        color: #1565c0 !important;
        border: 1px solid #bbdefb;
    }
    #penyakitTable .badge.badge-secondary {
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Data Master &raquo; Manajemen Penyakit</h3>
                <div class="card-tools">
                    <a href="{{ route('master.penyakit.create') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="input-group input-group-sm ml-2" style="width: 220px;">
                        ...
                        <input type="text" id="searchBox" class="form-control" placeholder="kata kunci pencarian" aria-controls="penyakitTable">
                    </div>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div class="table-responsive">
                    <table id="penyakitTable" class="table table-bordered table-striped table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th width="120" class="text-center">Kode Penyakit</th>
                                <th width="140" class="text-center">Kelompok Penyakit</th>
                                <th>Nama Penyakit</th>
...
<script>
    $(function () {
        var table = $('#penyakitTable').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "{{ route('master.penyakit.grid') }}",
                "type": "GET",
            ...
            "columns": [
                ...
                { "data": "kode_penyakit", "className": "text-center" },
                {
                    "data": "kelompok_penyakit",
                    "className": "text-center",
                    "render": function (data, type, row) {
                        var badgeClass = row.kelompok_badge || 'badge-secondary';
                        return '<span class="badge ' + badgeClass + '">' + (data || '-') + '</span>';
                    }
                },
                { "data": "nama_penyakit" },
                { "data": "ket_penyakit", "defaultContent": "-" },
                {
                ...
                },
                {
                    ...
                    "width": "90",
                    "render": function (data) {
                        var editUrl = '{{ url("admin/master/penyakit/edit") }}/' + data;
                        var deleteUrl = '{{ url("admin/master/penyakit/delete") }}/' + data;
        ...
        $('#penyakitTable tbody').on('change', '.checkItem', function () {
            var id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $('#checkAll').on('change', function () {
            ...
            $('#penyakitTable tbody .checkItem').prop('checked', isChecked);
            syncCheckAllState();
        });

        // Terapkan ulang status pilihan setelah DataTables redraw
        table.on('draw', function () {
            $('#penyakitTable tbody .checkItem').each(function () {
                var id = $(this).val();
    ...
    function hapusData() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('master.penyakit.delete') }}",
