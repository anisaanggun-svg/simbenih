...
@section("title", "Master Satuan - Data Master")
...
<style>
    #satuanTable th { white-space: nowrap; }
    #satuanTable td { vertical-align: middle; padding: 8px 10px; }
    .card-tools {
        display: flex;
        gap: 0.5rem;
        flex-wrap: wrap;
    ...
    #satuanTable .badge.badge-success {
        background-color: #dff3e1 !important;
        color: #2e7d32 !important;
        border: 1px solid #c8e6c9;
    }
    #satuanTable .badge.badge-danger {
        background-color: #fde2e2 !important;
        color: #c62828 !important;
        border: 1px solid #efcfcf;
    }
    /* Soft colors for jenis satuan badges */
    #satuanTable .badge.badge-primary {
        background-color: #dbe7ff !important;
        color: #1a3d8f !important;
        border: 1px solid #c5d6f5;
    }
    #satuanTable .badge.badge-info {
        background-color: #d3f1f7 !important;
        color: #0c6c84 !important;
        border: 1px solid #b9e4ed;
    }
    #satuanTable .badge.badge-warning {
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Data Master &raquo; Manajemen Satuan</h3>
                <div class="card-tools">
                    <a href="{{ route('master.satuan.create') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="input-group input-group-sm ml-2" style="width: 220px;">
                        ...
                        <input type="text" id="searchBox" class="form-control" placeholder="kata kunci pencarian" aria-controls="satuanTable">
                    </div>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div class="table-responsive">
                    <table id="satuanTable" class="table table-bordered table-striped table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th width="120" class="text-center">Kode Satuan</th>
                                <th>Nama Satuan</th>
                                <th width="140" class="text-center">Jenis Satuan</th>
                                <th width="100" class="text-center">Pengali</th>
...
<script>
    $(function () {
            ...
            "columns": [
                ...
                { "data": "nama_satuan" },
                {
                    ...
                    "render": function (data, type, row) {
                        var badgeClass = row.jenis_satuan_badge || 'badge-secondary';
                        var label = row.jenis_satuan_label || (data || '-');
                ...
                },
                {
                    ...
                    "width": "90",
                    "render": function (data) {
                        var editUrl = '{{ url("admin/master/satuan/edit") }}/' + data;
                        var deleteUrl = '{{ url("admin/master/satuan/delete") }}/' + data;
        ...
        $('#satuanTable tbody').on('change', '.checkItem', function () {
            var id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $('#checkAll').on('change', function () {
            ...
            $('#satuanTable tbody .checkItem').prop('checked', isChecked);
            syncCheckAllState();
        });

        // Terapkan ulang status pilihan setelah DataTables redraw
        table.on('draw', function () {
            $('#satuanTable tbody .checkItem').each(function () {
                var id = $(this).val();
        ...
        $('#btnConfirmHapus').on('click', function () {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('master.satuan.delete') }}",
