...
@section("title", "Edit Master Daftar User - Data Master")
    ...
    <div class="col-md-10 offset-md-1">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Form Edit Data Master Daftar User</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                @if($errors->any())
                ...
                <div class="info mb-3">
                    <p class="text-muted">User melakukan pengisian form untuk mengubah data user aplikasi. Kosongkan password jika tidak ingin mengubahnya.</p>
                </div>
                ...
                <form action="{{ route('master.daftar-user.update', $user->id) }}" method="POST">
                    @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select name="wewenang_data" id="wewenang_data"
                                    ...
                                    <option value="1" {{ old('wewenang_data', $user->wewenang_data) == '1' ? 'selected' : '' }}>Pangan</option>
                                    <option value="2" {{ old('wewenang_data', $user->wewenang_data) == '2' ? 'selected' : '' }}>Hortikultura</option>
                                    <option value="3" {{ old('wewenang_data', $user->wewenang_data) == '3' ? 'selected' : '' }}>Perkebunan</option>
                                    <option value="4" {{ old('wewenang_data', $user->wewenang_data) == '4' ? 'selected' : '' }}>Semua Golongan</option>
                                    <option value="5" {{ old('wewenang_data', $user->wewenang_data) == '5' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                                @error('wewenang_data')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_pegawai" id="id_pegawai"
                                    ...
                                    @foreach($pegawai as $p)
                                        <option value="{{ $p->id }}" {{ old('id_pegawai', $user->id_pegawai) == $p->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_satgas" id="id_satgas"
                                    ...
                                    @foreach($satgasList as $s)
                                        <option value="{{ $s->id_satgas }}" {{ old('id_satgas', $user->id_satgas) == $s->id_satgas ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select name="kode_kabupaten" id="kode_kabupaten"
                                    ...
                                    @foreach($kabupaten as $k)
                                        <option value="{{ $k->kode_kabupaten }}" {{ old('kode_kabupaten', $user->kode_kabupaten) == $k->kode_kabupaten ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_komoditas" id="id_komoditas"
                                    ...
                                    @foreach($komoditas as $km)
                                        <option value="{{ $km->id }}" {{ old('id_komoditas', $user->id_komoditas) == $km->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <a href="{{ route('master.daftar-user.index') }}" class="btn btn-light">
...
<script>
    ...
    // sesuai role (mengikuti sistem sumber).
