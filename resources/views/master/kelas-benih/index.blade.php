...
@section("title", "Master Kelas Benih - Data Master")
...
<style>
    #kelasBenihTable th { white-space: nowrap; }
    #kelasBenihTable td { vertical-align: middle; padding: 8px 10px; }
    .card-tools {
        display: flex;
        gap: 0.5rem;
        flex-wrap: wrap;
    ...
    #kelasBenihTable .badge.badge-success {
        background-color: #dff3e1 !important;
        color: #2e7d32 !important;
        border: 1px solid #c8e6c9;
    }
    #kelasBenihTable .badge.badge-danger {
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Data Master &raquo; Manajemen Kelas Benih</h3>
                <div class="card-tools">
                    <a href="{{ route('master.kelas-benih.create') }}" class="btn btn-success btn-sm">
                    ...
                    <div class="input-group input-group-sm ml-2" style="width: 220px;">
                        ...
                        <input type="text" id="searchBox" class="form-control" placeholder="kata kunci pencarian" aria-controls="kelasBenihTable">
                    </div>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div class="table-responsive">
                    <table id="kelasBenihTable" class="table table-bordered table-striped table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th width="180" class="text-center">Nama Grup Kelas Benih</th>
                                <th width="150" class="text-center">Kode Kelas Benih</th>
                                <th width="200">Nama Kelas Benih</th>
...
<script>
    $(function () {
        var table = $('#kelasBenihTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "{{ route('master.kelas-benih.grid') }}",
                "type": "GET",
            ...
            "columns": [
                ...
                { "data": "nama_grup_kelas_benih", "className": "text-center" },
                { "data": "kode_kb", "className": "text-center" },
                { "data": "nama_kb" },
                {
                    "data": "status_kelas_benih",
                    "className": "text-center",
                ...
                },
                {
                    ...
                    "width": "90",
                    "render": function (data) {
                        var editUrl = '{{ url("admin/master/kelas-benih/edit") }}/' + data;
                        var deleteUrl = '{{ url("admin/master/kelas-benih/delete") }}/' + data;
        ...
        $('#kelasBenihTable tbody').on('change', '.checkItem', function () {
            var id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $('#checkAll').on('change', function () {
            ...
            $('#kelasBenihTable tbody .checkItem').prop('checked', isChecked);
            syncCheckAllState();
        });

        // Terapkan ulang status pilihan setelah DataTables redraw
        table.on('draw', function () {
            $('#kelasBenihTable tbody .checkItem').each(function () {
                var id = $(this).val();
    ...
    function hapusData() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('master.kelas-benih.delete') }}",
