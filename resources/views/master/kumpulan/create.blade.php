...
@section("title", "Tambah Master Kumpulan - Data Master")
    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Form Input Data Master Kumpulan</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                @if($errors->any())
                ...
                <div class="info mb-3">
                    <p class="text-muted">User melakukan pengisian form untuk menambah data kumpulan.</p>
                </div>
                ...
                <form action="{{ route('master.kumpulan.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="kode_golongan" id="kode_golongan"
                               ...
                               value="{{ old('kode_golongan') }}"
                               maxlength="5" placeholder="Contoh: SR, KCP, UBP" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_golongan" id="nama_golongan"
                               ...
                               placeholder="Contoh: Serealia, Aneka Kacang" required>
                        @error('nama_golongan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="komoditas_id" id="komoditas_id"
                            ...
                            @foreach($kodeList ?? [] as $id => $nama)
                                <option value="{{ $id }}" {{ old('komoditas_id') == $id ? 'selected' : '' }}>{{ $nama }}</option>
                            @endforeach
                            @foreach($komoditasList ?? [] as $k)
                                <option value="{{ $k->id }}" {{ (string) old('komoditas_id') === (string) $k->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_golongan" id="jenis_golongan"
                            ...
                            @foreach(\App\Models\Golongan::jenisOptions() as $key => $label)
                                <option value="{{ $key }}" {{ old('jenis_golongan', \App\Models\Golongan::JENIS_LAINNYA) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status_golongan" id="status_golongan"
                            ...
                            <option value="1" {{ old('status_golongan', '1') == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status_golongan') == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
